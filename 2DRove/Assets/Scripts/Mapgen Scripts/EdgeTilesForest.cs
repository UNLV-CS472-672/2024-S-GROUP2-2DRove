...
namespace EdgeTilesForest
{
    public class AssignEdgesForest : MonoBehaviour
    {
        ...
        private HashSet<Vector2> borderPositions; 

        public void SetTilePositions(HashSet<Vector2> positions) 
        {
            tilePositions = new HashSet<Vector2>(positions); 
        }
        ...
        public void SetBorderPositions(HashSet<Vector2> positions) 
        {
            borderPositions = new HashSet<Vector2>(positions);
        }
        ...
        public void SwapTile(Vector2 position, GameObject newTilePrefab) {
            if (tileObjects.TryGetValue(position, out GameObject oldTile)) {
                ...
                GameObject newTile = Instantiate(newTilePrefab, new Vector3(position.x * scale * 4, position.y * scale * 2, 0), Quaternion.identity);
        ...
        public void BorderingCorners() {
            foreach (var tilePos in tilePositions) {
                ...
                if (tileType == TileType.WPeninsula || tileType == TileType.NPeninsula ||
                    ...
                    Vector2 lShapePosition = Vector2.zero;
                    GameObject cornerPrefab = null;
                    bool enter = true;

                    if ((tileType == TileType.WPeninsula || tileType == TileType.NWCorner) && enter) {
                        lShapePosition = tilePos + new Vector2(0f, 2f); // Position to the northeast
                        ...
                        if (lShapePosition != Vector2.zero && !borderPositions.Contains(lShapePosition)) {
                            Vector2 basePosition = tilePos + new Vector2(1f, 1f);
                    ...
                    if ((tileType == TileType.EPeninsula || tileType == TileType.NECorner) && enter) {
                        lShapePosition = tilePos + new Vector2(-2f, 0f); // Position to the northwest
                    ...
                    if ((tileType == TileType.SPeninsula || tileType == TileType.SWCorner) && enter) {
                        lShapePosition = tilePos + new Vector2(-2f, 0f); // Position to the southwest
                    ...
                    if ((tileType == TileType.SPeninsula || tileType == TileType.SECorner) && enter) {
                        lShapePosition = tilePos + new Vector2(0f, -2f); // Position to the southeast
                    ...
                    if ((tileType == TileType.NPeninsula || tileType == TileType.NECorner) && enter) {
                        lShapePosition = tilePos + new Vector2(2f, 0f); // Position to the southeast
                        cornerPrefab = CornerNEPrefab;
        ...
        void OverlayPrefabs(Vector2 position, GameObject basePrefab, GameObject overlayPrefab) {
            ...
            if (tileObjects.TryGetValue(position, out GameObject oldTile)) {
                ...
                GameObject overlayTile = Instantiate(overlayPrefab, new Vector3(position.x * scale * 4, position.y * scale * 2, 1f), Quaternion.identity);
